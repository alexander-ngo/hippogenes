...
.. py:function:: hippogenes.load_expression(genes=None, data_dir=None, force_download=False)
   ...
   :returns expression: A pandas DataFrame containing gene expression values. If genes is specified, only the requested genes are returned. The row ordering matches the hippocampal surface mesh bundled with the atlas.
   :type expression: pandas.DataFrame
   
   :returns ValueError: If one or more gene names are not present in the atlas
   :returns ConnectionError: If the atlas data cannot be downloaded and is not available in the local cache
